...
class ActorCritic(nn.Module):
    ...
    def get_action_and_value(self,obs, action=None):
        # MUST return: action, log_prob, entropy, value
        # (action=None -> sample a new one; action=given -> just score it)

        action_probs, action_std,state_value=self.forward(obs)

        dist = Normal(action_probs,action_std)
...
class RolloutBuffer:
    def __init__(self , num_steps:int ,obs_dim:int , action_dim:int ):
        ...
        self.ptr = 0  # tracks which slot to fill next
...
def compute_gae(rewards , values, dones, last_value,gamma:float, lam:float):
    n=len(rewards)
    advantages = gamma * last_value - values
    last_gae=0.0
    ...
    res=advantages + values
    return advantages , res
...
def ppo_train(env, total_timestamps: int, num_steps: int,
    ...
    while timesteps_done < total_timestamps:
        ...
        for step in range(num_steps):
            ...
            if done:
                ...
                print(f"timesteps={timesteps_done + step}  episode_reward={episode_reward:.2f}")
                episode_reward = 0.0
                obs, _ = env.reset()
        ...
        ppo_update(model, optimizer, buffer, advantages, returns,
                   clip_eps=0.2, vf_coef=0.5, ent_coef=0.01, num_epochs=10, batch_size=64)
...
def evaluate(model, env, n_episodes: int):
    ...
    for _ in range(n_episodes):
        ...
        while not done:
            obs_t = torch.as_tensor(obs, dtype=torch.float32).unsqueeze(0)
            with torch.no_grad():
                mean, std, _ = model.forward(obs_t)   # deterministic: use the MEAN, not a sample
